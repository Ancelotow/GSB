...
class RegistrationController extends Controller
{
    ...
    public function confirmedAction(Request $request)
    {
        ...
        $form = $this->createForm(visiteurType::class , $visiteur, ['user'=>$user]);

        if($request->isMethod('POST')){
            $form->handleRequest($request);
            if($form->isValid()){
                $visiteur->setUser($user);
                $em = $this->getDoctrine()->getManager();
                $em->persist($visiteur);
                $em->flush();
                $request->getSession()->getFlashBag()->add('notice', 'Etat bien enregistre.');
                return $this->redirectToRoute('gsb_accueil_accueil1');
            }

        }
        $id = $user->getId();
        return $this->render('GSBAccueilBundle:Default:visiteur.html.twig', array(
            'idUser' => $id,
            'form'=>$form->createView()
        ));
    }
